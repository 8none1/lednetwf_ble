...
| Context | Range | Convert to HA |
|---------|-------|---------------|
| Commands (effect) | 0-100% | × 2.55 |
| Commands (RGB) | 0-255 | Direct |
| State (white mode) | 0-100% | × 2.55 |
| State (RGB mode) | Derive from HSV | Calculate |

### Speed

| Device Type | Range | Encoding | Conversion |
|-------------|-------|----------|------------|
| FillLight (0x1D) | 0-100 | Direct | None |
| Symphony (0xA1-0xA9) | 1-31 | **Inverted** | `31 - (ui × 30 / 100)` |
| Legacy RGB (0x61) | 1-31 | **Inverted** | `1 + (30 × (1 - ui/100))` |
| Standard strips | 0-100 | Direct | None |

**Inverted speed formula:**
- UI 100% (fast) → protocol 1
- UI 50% (medium) → protocol 16
- UI 0% (slow) → protocol 31
